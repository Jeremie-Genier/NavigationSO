...
public class GestNiveau : MonoBehaviour
{
    ...
    [SerializeField] private InfosJoueur _iJoueur; //infos du joueur    (type SO = nom de la classe du script)
    [SerializeField] private InfosNavig _iNav; //infos de la navigation
    ...
    void Awake()
    {
        ...
        if(_instance == null) //condition habituelle du Singleton
        {
            _instance = this;
        }
        else //else
    ...
    {
        _champNiveau.text = "NIVEAU " + _iNav.niveau; //affichage de la concatenation du mot et de l'info
    }
    ...
    public void RecupererTresor(int points) //ne pas oublier d'ajouter le parametre
    {
        ...
        if(_nbTresorsRecup >= _nbTresorsIni) //si tous les trésors ont ete ramasses
        {
            _iNav.AllerNiveauSuivant(); //changer de scene avec la methode des infos de navigation
        }
    }
    ...
    public void PerdreVie()
    {
        ...
        if(_iJoueur.vie <= 0) //si mort pour vrai, aller a la scene de fin
        {
            _iNav.AllerFin();
        }
    }
