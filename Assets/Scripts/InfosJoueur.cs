...
public class InfosJoueur : ScriptableObject //preciser que c'est un S.O.
{
    ...
    [SerializeField] int _scoreIni = 0; //int, score initial (0 par defaut)
    ...
    [SerializeField] int _score; //int, score du joueur durant la partie
    //FIN DES CHAMPS SERIALISES

    [HideInInspector] public string _nomIntermediaire =""; //(cache ds l'inspecteur) nom intermediaire, correspond au nom inscrit dans le champ par le joueur (avant le jeu)
